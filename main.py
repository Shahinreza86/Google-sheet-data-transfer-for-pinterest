...
# জেমিনি সেটআপ - আপনার তালিকার (image_87b6f5.png) 'Gemini Flash-Lite Latest' ব্যবহার করা হয়েছে
...
def process_data():
    ...
    for i, row in enumerate(all_rows[1:], start=2):
        # কলাম C-তে লিঙ্ক থাকলে এবং কলাম A ফাঁকা থাকলে কাজ করবে
        if len(row) > 2 and "amazon" in row[2].lower() and (len(row) < 1 or not row[0].strip()):
            product_link = row[2].strip()
            print(f"--- সারি {i} প্রসেস হচ্ছে ---")
            
            # সরাসরি আমাজন থেকে সঠিক ছবির লিঙ্ক আনা
            actual_image_url = get_amazon_data(product_link)
            
            try:
                prompt = f"Analyze: {product_link}. Output: NAME: [Full Product Name], TITLE: [Short Pinterest Title], BOARD: [Select from {BOARDS}]. One per line."
                response = model.generate_content(prompt)
                lines = response.text.split('\n')
                
                p_name = next((l.split('NAME:')[1] for l in lines if 'NAME:' in l), "Product").strip()
                p_title = next((l.split('TITLE:')[1] for l in lines if 'TITLE:' in l), "Catchy Item").strip()
                p_board = next((l.split('BOARD:')[1] for l in lines if 'BOARD:' in l), BOARDS[0]).strip()

                # কলাম অনুযায়ী শিট আপডেট
